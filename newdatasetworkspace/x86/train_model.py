...
# create a folder to store resized eye images
resize_img = './calibration/'
if os.path.exists(resize_img) is False:
	os.mkdir(resize_img)

# Go through the example images and resize to 64 x64
eye_pathes = ['./eye/open', './eye/close']
...
for eye_path in eye_pathes:
	eye_list = os.listdir(eye_path)
	for i in range(0, len(eye_list)):
		img_path = os.path.join(eye_path, eye_list[i])
...
def _read_img_function(imagepath): 
	...
	image_resize = tf.image.resize_images(image_tensor, [64, 64])
...
model = keras.Sequential([
    ...
    keras.layers.Conv2D(32, (3,3), padding="same", activation=tf.nn.relu, input_shape=(64, 64, 3)),
...
model = keras.Sequential([
	#keras.layers.Flatten(input_shape=(64, 64, 3)),
	keras.layers.Conv2D(32, (3,3), padding="same", input_shape=(64, 64, 3)),
	keras.layers.MaxPool2D(pool_size=(2,2)),
...
model.save("eye_classification_weights.h5")
